...
def extract_amount(text):
    # 🛠️ 신협 전용: '입금/출금' 뒤에 나오는 금액을 추출합니다.
    match = re.search(r'(?:입금|출금)\s*([\d,]+)원?', text)
    if match:
...
async def handle_sms(request: Request):
    ...
    # 🛠️ 신협 전용: '잔액' 글자 바로 뒤에 붙어오는 숫자를 추출합니다.
    ...
    if trans_type:
        ...
        # 🛠️ 신협 전용 이름 추출: 금액(원)과 '잔액' 글자 사이에 있는 모든 텍스트를 잡아냅니다.
        name_match = re.search(r'(?:입금|출금)\s*[\d,]+원?\s+(.+?)\s*잔액', sms_message)
